...
function patchIndex() {
  ...
  // v7.15 LINE GROUP TRACEABILITY
  // groupId ของ LINE คือ tenant/workspace อยู่แล้ว จึงตามกลุ่มย้อนหลังได้โดยไม่แก้ Sheet schema
  s = mustReplace(s, "    if(!write)return [\"/api/expenses\",\"/api/batches\",\"/api/settings\",\"/api/workspace-links\",\"/api/subscription\",\"/api/businesses\",\"/api/accounting/today\",\"/api/accounting/whoami\"].some(p=>path===p||path.startsWith(p+\"/\"));", "    if(!write)return [\"/api/expenses\",\"/api/batches\",\"/api/line-groups\",\"/api/settings\",\"/api/workspace-links\",\"/api/subscription\",\"/api/businesses\",\"/api/accounting/today\",\"/api/accounting/whoami\"].some(p=>path===p||path.startsWith(p+\"/\"));", 'approver can read line groups');

  const tenantTitleAnchorV715 = "async function tenantTitle(env, source) {\n  try {\n    if (source.groupId) {\n      const r = await fetch(`https://api.line.me/v2/bot/group/${source.groupId}/summary`, {\n        headers: { Authorization: `Bearer ${env.LINE_ACCESS_TOKEN}` } });\n      if (r.ok) return (await r.json()).groupName || \"\";\n    } else if (source.userId) {\n      const r = await fetch(`https://api.line.me/v2/bot/profile/${source.userId}`, {\n        headers: { Authorization: `Bearer ${env.LINE_ACCESS_TOKEN}` } });\n      if (r.ok) return (await r.json()).displayName || \"\";\n    }\n  } catch {}\n  return \"\";\n}";
  const lineGroupHelpersV715 = "\nfunction lineSourceTypeFromTenant(tenant = \"\") {\n  const id = String(tenant || \"\");\n  if (/^C/.test(id)) return \"group\";\n  if (/^R/.test(id)) return \"room\";\n  if (/^U/.test(id)) return \"direct\";\n  return \"unknown\";\n}\n\nasync function getLineSourceMeta(env, tenant, { refresh = false, fallbackName = \"\" } = {}) {\n  const id = String(tenant || \"\").trim();\n  const sourceType = lineSourceTypeFromTenant(id);\n  const cacheKey = `linegroupmeta:v1:${id}`;\n  const cached = await env.KV.get(cacheKey, \"json\").catch(() => null);\n  const cachedAge = cached?.checkedAt ? Date.now() - Date.parse(cached.checkedAt) : Infinity;\n\n  if (!refresh && cached && cachedAge < 6 * 60 * 60 * 1000) {\n    return { ...cached, fallbackName: fallbackName || cached.fallbackName || \"\" };\n  }\n\n  let liveName = \"\";\n  let connected = cached?.connected === true;\n  let error = \"\";\n\n  if (sourceType === \"group\" && env.LINE_ACCESS_TOKEN) {\n    try {\n      const response = await fetch(`https://api.line.me/v2/bot/group/${encodeURIComponent(id)}/summary`, {\n        headers: { Authorization: `Bearer ${env.LINE_ACCESS_TOKEN}` },\n      });\n      if (response.ok) {\n        const body = await response.json().catch(() => ({}));\n        liveName = String(body.groupName || \"\").trim();\n        connected = true;\n      } else {\n        connected = false;\n        error = `LINE ${response.status}`;\n      }\n    } catch (e) {\n      error = String(e?.message || e).slice(0, 160);\n    }\n  } else if (sourceType === \"direct\") {\n    connected = true;\n  }\n\n  const result = {\n    sourceId: id,\n    sourceType,\n    groupId: sourceType === \"group\" ? id : \"\",\n    groupName: liveName || String(fallbackName || cached?.groupName || \"\").trim() ||\n      (sourceType === \"group\" ? `LINE Group \u00b7\u00b7\u00b7${id.slice(-6)}` :\n       sourceType === \"room\" ? `LINE Room \u00b7\u00b7\u00b7${id.slice(-6)}` :\n       sourceType === \"direct\" ? \"\u0e41\u0e0a\u0e17\u0e2a\u0e48\u0e27\u0e19\u0e15\u0e31\u0e27\" : `Workspace \u00b7\u00b7\u00b7${id.slice(-6)}`),\n    connected,\n    error,\n    checkedAt: new Date().toISOString(),\n  };\n  await env.KV.put(cacheKey, JSON.stringify(result), { expirationTtl: 60 * 60 * 24 * 30 });\n  return result;\n}\n\nfunction lineClaimStatusKey(row = {}) {\n  const batchStatus = String(row.batchStatus || \"\").trim();\n  const status = String(row.status || \"\").trim();\n  if (row.paid === true || batchStatus === \"\u0e08\u0e48\u0e32\u0e22\u0e41\u0e25\u0e49\u0e27\" || String(row.reimbursedAt || \"\").trim()) return \"paid\";\n  if (/\u0e44\u0e21\u0e48\u0e2d\u0e19\u0e38\u0e21\u0e31\u0e15\u0e34|\u0e22\u0e01\u0e40\u0e25\u0e34\u0e01/.test(batchStatus) || /\u0e44\u0e21\u0e48\u0e2d\u0e19\u0e38\u0e21\u0e31\u0e15\u0e34|\u0e22\u0e01\u0e40\u0e25\u0e34\u0e01/.test(status)) return \"rejected\";\n  if (/\u0e41\u0e01\u0e49\u0e44\u0e02|\u0e15\u0e35\u0e01\u0e25\u0e31\u0e1a/.test(batchStatus) || /\u0e41\u0e01\u0e49\u0e44\u0e02|\u0e15\u0e35\u0e01\u0e25\u0e31\u0e1a/.test(status)) return \"correction\";\n  if (/\u0e23\u0e2d\u0e42\u0e2d\u0e19|\u0e23\u0e2d\u0e2b\u0e25\u0e31\u0e01\u0e10\u0e32\u0e19/.test(batchStatus) || /\u0e23\u0e2d\u0e42\u0e2d\u0e19|\u0e23\u0e2d\u0e2b\u0e25\u0e31\u0e01\u0e10\u0e32\u0e19/.test(status)) return \"payment\";\n  return \"review\";\n}\n\nasync function getLineGroupsOverview(env, currentTenant, { refresh = false } = {}) {\n  const rootTenant = await getAccountRoot(env, currentTenant);\n  const cacheKey = `linegroups:v1:${rootTenant}`;\n  if (!refresh) {\n    const cached = await env.KV.get(cacheKey, \"json\").catch(() => null);\n    if (cached?.ok && Array.isArray(cached.rows)) return cached;\n  }\n\n  const account = await ensureBusinessAccount(env, rootTenant);\n  const rows = [];\n\n  for (let i = 0; i < account.businesses.length; i++) {\n    const tenant = account.businesses[i];\n    const sheetId = await env.KV.get(`tenant:${tenant}`);\n    const businessMeta = await readBusinessMeta(env, tenant).catch(() => ({}));\n    let token = null;\n    let settings = {};\n    let expenses = [];\n\n    if (sheetId) {\n      token = await getUserToken(env, tenant).catch(() => null);\n      if (token) {\n        settings = await readSettings(env, sheetId, token).catch(() => ({}));\n        expenses = await readExpenses(env, sheetId, token).catch(() => []);\n      }\n    }\n\n    const businessName =\n      settingValue(settings, \"company_name\") ||\n      String(businessMeta.name || \"\").trim() ||\n      (i === 0 ? \"\u0e18\u0e38\u0e23\u0e01\u0e34\u0e08\u0e2b\u0e25\u0e31\u0e01\" : `\u0e18\u0e38\u0e23\u0e01\u0e34\u0e08 ${i + 1}`);\n\n    const source = await getLineSourceMeta(env, tenant, {\n      refresh,\n      fallbackName: String(businessMeta.lineGroupName || businessMeta.name || \"\").trim(),\n    });\n\n    const claimRows = (expenses || []).filter((row) =>\n      String(row.type || \"\u0e23\u0e32\u0e22\u0e08\u0e48\u0e32\u0e22\") !== \"\u0e23\u0e32\u0e22\u0e23\u0e31\u0e1a\" && String(row.status || \"\") !== \"\u0e25\u0e1a\u0e41\u0e25\u0e49\u0e27\"\n    );\n\n    const counts = { review: 0, correction: 0, payment: 0, paid: 0, rejected: 0 };\n    let openAmount = 0;\n    let paidAmount = 0;\n\n    for (const row of claimRows) {\n      const key = lineClaimStatusKey(row);\n      counts[key] = (counts[key] || 0) + 1;\n      const amount = Number(row.amount || 0);\n      if (key === \"paid\") paidAmount += amount;\n      else if (key !== \"rejected\") openAmount += amount;\n    }\n\n    rows.push({\n      tenant,\n      businessName,\n      isRoot: tenant === rootTenant,\n      isCurrent: tenant === currentTenant,\n      sheetId: sheetId || \"\",\n      ...source,\n      summary: {\n        totalCount: claimRows.length,\n        openCount: counts.review + counts.correction + counts.payment,\n        openAmount,\n        paidCount: counts.paid,\n        paidAmount,\n        reviewCount: counts.review,\n        correctionCount: counts.correction,\n        paymentCount: counts.payment,\n        rejectedCount: counts.rejected,\n        lastActivityAt: claimRows[0]?.createdAt || \"\",\n      },\n      claims: claimRows.slice(0, 30).map((row) => ({\n        id: row.id || \"\",\n        date: row.dateISO || row.dateText || row.date || \"\",\n        createdAt: row.createdAt || \"\",\n        payerName: row.payerName || row.sender || \"\",\n        vendor: row.vendor || \"\",\n        note: row.note || \"\",\n        amount: Number(row.amount || 0),\n        status: row.batchStatus || row.status || \"\",\n        statusKey: lineClaimStatusKey(row),\n        batchDocId: row.batchDocId || row.batchNo || \"\",\n        paid: row.paid === true,\n      })),\n    });\n  }\n\n  const result = {\n    ok: true,\n    rootTenant,\n    currentTenant,\n    groupCount: rows.filter((row) => row.sourceType === \"group\").length,\n    roomCount: rows.filter((row) => row.sourceType === \"room\").length,\n    workspaceCount: rows.length,\n    rows,\n    refreshedAt: new Date().toISOString(),\n  };\n  await env.KV.put(cacheKey, JSON.stringify(result), { expirationTtl: 60 });\n  return result;\n}\n";
  s = mustReplace(
    s,
    tenantTitleAnchorV715,
    tenantTitleAnchorV715 + "\n" + lineGroupHelpersV715,
    'line group helpers'
  );
  ...
  s = mustReplace(
    ...
    "        if (url.pathname === \"/api/line-groups\") {\n          if (![\"owner\", \"accountant\", \"approver\"].includes(access.role)) {\n            return cors(json({ ok:false, error:\"forbidden\", message:\"\u0e2a\u0e34\u0e17\u0e18\u0e34\u0e4c\u0e19\u0e35\u0e49\u0e44\u0e21\u0e48\u0e2a\u0e32\u0e21\u0e32\u0e23\u0e16\u0e14\u0e39\u0e20\u0e32\u0e1e\u0e23\u0e27\u0e21 LINE \u0e01\u0e25\u0e38\u0e48\u0e21\u0e17\u0e31\u0e49\u0e07\u0e2b\u0e21\u0e14\u0e44\u0e14\u0e49\" }, 403));\n          }\n          const info = await getLineGroupsOverview(env, key, { refresh: url.searchParams.get(\"refresh\") === \"1\" });\n          if (access.role === \"owner\") {\n            for (const row of info.rows || []) {\n              row.dashboardUrl = await dashUrl(env, row.tenant);\n            }\n          }\n          return cors(json(info));\n        }\n\n" + '        if (url.pathname === "/api/businesses") {',
    'line groups endpoint'
  );

  s = mustReplace(s, "        if (url.pathname === \"/api/expenses\") {\n          const rows = await readExpenses(env, sheetId, token);", "        if (url.pathname === \"/api/expenses\") {\n          const baseRows = await readExpenses(env, sheetId, token);\n          const sourceGroup = await getLineSourceMeta(env, key, {\n            fallbackName: String((await readBusinessMeta(env, key).catch(() => ({}))).name || \"\").trim(),\n          });\n          const rows = baseRows.map((row) => ({\n            ...row,\n            sourceGroupId: sourceGroup.groupId || sourceGroup.sourceId || key,\n            sourceGroupName: sourceGroup.groupName || \"\",\n            sourceType: sourceGroup.sourceType || \"\",\n          }));", 'tag expenses with source group');
  s = mustReplace(s, "              \"slipNo\",\"transferor\",\"claimPdfUrl\",\"receiptPdfUrl\",\"duplicateStatus\",\"duplicateOf\"\n            ];", "              \"slipNo\",\"transferor\",\"claimPdfUrl\",\"receiptPdfUrl\",\"duplicateStatus\",\"duplicateOf\",\n              \"sourceGroupId\",\"sourceGroupName\",\"sourceType\"\n            ];", 'dashboard source group fields');
  s = mustReplace(s, "        if (url.pathname === \"/api/batches\") {\n          return cors(json(await getBatchDashboard(env, sheetId, token)));\n        }", "        if (url.pathname === \"/api/batches\") {\n          const batchDashboard = await getBatchDashboard(env, sheetId, token);\n          const sourceGroup = await getLineSourceMeta(env, key, {\n            fallbackName: String((await readBusinessMeta(env, key).catch(() => ({}))).name || \"\").trim(),\n          });\n          return cors(json({ ...batchDashboard, sourceGroup }));\n        }", 'tag reimbursement dashboard with source group');
